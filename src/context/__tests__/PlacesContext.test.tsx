...
jest.mock('../../db/database', () => ({
  ...
  updateLocalSavedPlaceNote: (...args: any[]) => mockUpdateLocalSavedPlaceNote(...args),
...
describe('PlacesContext', () => {
  ...
  /**
   * Scenario 7.1.3 — [BUG] TOCTOU race: concurrent saves bypass duplicate check
   *
   * savePlace (PlacesContext.tsx:204-219) does:
   *   1. isDuplicatePlace(userId, googlePlaceId)  ← check
   *   2. ... async work ...
   *   3. insertLocalSavedPlace(...)                ← insert
   *
   * If two savePlace calls run concurrently for the same google_place_id,
   * both can pass the isDuplicatePlace check (step 1) before either reaches
   * the insert (step 3). Both then insert + push to remote.
   *
   * INSERT OR IGNORE means the second local insert is a silent no-op,
   * but BOTH calls still push to Supabase (uploadSavedPlace), creating
   * duplicate server-side entries.
   */
  it('7.1.3: concurrent savePlace calls both pass isDuplicatePlace for the same google_place_id', async () => {
    const mockUploadSavedPlace = require('../../services/supabaseService').uploadSavedPlace;

    // isDuplicatePlace always returns false (both calls see "not duplicate")
    ...
    // BUG: isDuplicatePlace was called twice with the same args, and both
    // returned false because neither insert had completed yet.
    expect(mockIsDuplicatePlace).toHaveBeenCalledTimes(2);
    expect(mockIsDuplicatePlace).toHaveBeenCalledWith('user-1', 'ChIJ123');

    // Both calls proceeded to insert locally
    expect(mockInsertLocalSavedPlace).toHaveBeenCalledTimes(2);

    // BUG: Both calls pushed to Supabase — creating duplicate server entries
    expect(mockUploadSavedPlace).toHaveBeenCalledTimes(2);
  });
});
